module.exports = {
    name: "disableforall",
    desc: "Disables a guild command for all roles",
    interactionHandler: async interaction => {
        ...
        let permissions = interaction.guild.roles.cache.map(role => {
            return {
                id: role.id,
                type: 'ROLE',
                permission: false
        ...
        .then(() => {
            interaction.editReply({ content: `Sucessfully disabled ${commandName} for all roles` });
        })
        .catch(err => {
